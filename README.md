...
- **Manage** — enter the event’s PIN to add/remove guests, open an individual guest’s postcard-style pass to save or share it as a PNG, print QR codes, send QR codes by email with Resend, rename the event, or change the PIN. Open **Event settings** to rename, change the PIN, or lock the page. Access remains unlocked in the current browser tab until it closes or you choose **Lock Manage page**. The first visitor to Manage for an older event without a PIN will be asked to set one.
